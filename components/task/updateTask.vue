...
export default {
    ...
    submitForm() {
      this.$validator.validateAll().then((result) => {
        if (result) {
          ...
          const index = data.findIndex((item) => item.id === this.taskData.id);
          if (index !== -1) {
            data[index] = this.taskData;
            nuxtStorage.localStorage.setData("TaskArray", data, 1, "d");
          }
          //   updateItemInLocalStorage(this.taskData);
          this.showModal = false;
          this.$emit("close");
        }
      });
  ...
  mounted() {
    ...
    this.taskData.id = this.taskId["id"];
    this.taskData.title = this.taskId["title"];
    this.taskData.priority = this.taskId["priority"];
    this.taskData.estimated_hour = this.taskId["estimated_hour"];
    this.taskData.description = this.taskId["description"];
